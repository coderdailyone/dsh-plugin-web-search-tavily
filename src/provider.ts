 ...
 * (`POST /search`). It maps `results[].content` to `snippet`, `published_date`
 * to `publishedAt`, drops entries without a non-blank snippet, and maps the
 * optional generated `answer` to `content` when `includeAnswer` is enabled.
 * @module dsh-plugin-web-search-tavily/provider
 */
...
/** Default retrieval depth: one credit per query on Tavily's pricing. */
export const TAVILY_DEFAULT_SEARCH_DEPTH = 'basic'

/** Default topic vertical. */
export const TAVILY_DEFAULT_TOPIC = 'general'

/** Attribution header sent on every request. Bump with the package version. */
const USER_AGENT = 'dsh-plugin-web-search-tavily/0.1.2'
...
export interface TavilySearchProviderOptions {
  ...
  baseURL: string
  /** Tavily's `search_depth`: `advanced` costs more credits and latency. */
  searchDepth: 'basic' | 'advanced'
  /** Tavily's `topic` vertical. */
  topic: 'general' | 'news'
  /** Request Tavily's generated answer and surface it as `WebSearchResult.content`. */
 ...
 * no portable snippet (an entry with no non-blank `content` is dropped — the
 * seam has no other field to derive a snippet from, and inventing one would
 ...
 *   ({@link mapTavilyResult}), and a non-blank `answer` becomes `content`.
 */
  ...
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        const detail = typeof parsed.detail === 'object' && parsed.detail !== null
          ? parsed.detail.error
          : typeof parsed.detail === 'string' ? parsed.detail : undefined
        const best = detail ?? parsed.error ?? parsed.message
        if (best !== undefined && best.length > 0) message = best
      } catch (error: unknown) {
        ...
        // malformed error body can only cost a richer provider message.
      }
